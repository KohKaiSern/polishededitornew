...
function reverseParsePlayer(
	...
	//Badges
	const badgeVal = parseInt(player.badges.map((badge) => (badge.owned ? '1' : '0')).join(''), 2)
		.toString(16)
		.padStart(4, '0');
	fileHex[addresses.wBadges] = badgeVal.slice(0, 2);
	fileHex[addresses.wBadges + 1] = badgeVal.slice(2);
